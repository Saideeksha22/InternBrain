...
def extract_massive_features(img):
    ...
    for d in distances:
        for a in angles:
            glcm = graycomatrix(img, distances=[d], angles=[a]a, symmetric=True, normed=True)
            for prop in props:
    ...
    for i in range(min(256, len(hog_features))):
...
base_path = r"C:\Users\saide\OneDrive\Desktop\Stroke_Detection-and-Segmentation-by-Using-CNN-ML\all_png_images"
...
print(" All features were extracted and saved to the all_features.csv file.")
