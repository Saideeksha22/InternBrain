...
# ---------- Dizin Ayarları ----------
root_dir = r'C:\Users\saide\OneDrive\Desktop\Stroke_Detection-and-Segmentation-by-Using-CNN-ML\all_png_images'  # Burayı kendi kullanıcı adına göre değiştir!
image_dir = root_dir
...
class StrokeDataset(Dataset):
    def __init__(self, image_root, transform=None):
        ...
        for cls in os.listdir(image_root):
            cls_folder = os.path.join(image_root, cls)
            if os.path.isdir(cls_folder):
                # Recursively find all images in all subfolders (PNG, OVERLAY, etc.)
                for img_path in glob.glob(os.path.join(cls_folder, '**', '*.*'), recursive=True):
